...
from python.async_pipeline import PipelineStage, PipelineResult, CompressionPipeline, compress_async
...
def test_compress_async_basic():
    ...
    async def _run():
        return await compress_async(
            Vectro(), np.random.randn(10, 64).astype(np.float32), mode="int8"
        )
