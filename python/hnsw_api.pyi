...
class HNSWIndex:
    ...
    def search(
        self, query: np.ndarray, k: int = 10, ef: int = 64
...
def build_hnsw_index(
    vectors: np.ndarray, M: int = 16, ef_construction: int = 200, space: str = "cosine"
) -> HNSWIndex: ...
def hnsw_search(
    index: HNSWIndex, query: np.ndarray, k: int = 10, ef: int = 64
) -> tuple[np.ndarray, np.ndarray]: ...
def recall_at_k(
    index: HNSWIndex, queries: np.ndarray, ground_truth: np.ndarray, k: int = 10, ef: int = 64
) -> float: ...
