...
use vectro_cli::{compress_auto, compress_nf4, compress_pq, compress_rq, compress_stream};
        ...
        let method = if let Ok(txt) = std::fs::read_to_string(&config_path) {
            if let Ok(json) = serde_json::from_str::<serde_json::Value>(&txt) {
                ...
                let force_int8_archs: std::collections::HashSet<&str> =
                    ["XLMRobertaModel", "RobertaModel", "E5Model"]
                        .iter()
                        .copied()
                        .collect();
...
fn execute_serve_command(port: u16) -> anyhow::Result<()> {
    tokio::runtime::Runtime::new()?.block_on(async { server::serve(port).await })
}
...
fn load_dataset_or_default(dataset_path: Option<&str>) -> Vec<vectro_lib::Embedding> {
    ...
    if let Some(path) = dataset_path {
        if let Ok(ds) = vectro_lib::EmbeddingDataset::load(path) {
        ...
        Commands::Bench {
            ...
            pb.set_style(
                ProgressStyle::with_template("{spinner} {msg}").unwrap_or_else(|_| {
                    unreachable!("hardcoded progress template is always valid")
                }),
            );
            ...
            let mut child = cmd
                .spawn()
                .map_err(|e| anyhow::anyhow!("failed to spawn cargo bench: {e}"))?;
            ...
            let status = child
                .wait()
                .map_err(|e| anyhow::anyhow!("bench wait failed: {e}"))?;
            pb.finish_and_clear();
            if !status.success() {
                        ...
                        if let Ok(entries) = fs::read_dir(&crit_dir) {
                            ...
                            if !rows.is_empty() {
                                ...
                                for (f, med, mean, unit) in &rows {
                                    ...
                                    println!(
                                        "{:<60} {:>12} {:>12} {:>8} {:>8}",
                                        f, med_s, mean_s, unit_s, delta_s
                                    );
                                }

                                // update history with latest medians
                                let mut new_hist: std::collections::HashMap<String, f64> =
                                    std::collections::HashMap::new();
                                for (f, med, _mean, _unit) in &rows {
                                    if let Some(m) = med {
                                        new_hist.insert(f.clone(), *m);
                                    }
                                }
                    ...
                    if let Some(dest) = save_report {
                        ...
                        if open_report {
                            ...
                            if let Err(e) = Command::new(opener)
                                .arg(target_copy.join("index.html"))
                                .spawn()
                            {
                        ...
                        while let Some(p) = stack.pop() {
                            if let Ok(entries) = std::fs::read_dir(&p) {
                                for en in entries.flatten() {
                                    ...
                                    if pp.is_dir() {
                                        stack.push(pp);
                                    } else if pp.file_name().and_then(|s| s.to_str())
                                        == Some("index.html")
                                    {
                                        index_opt = Some(pp);
                                        break;
    ...
    match v {
        Value::Object(map) => {
            ...
            for (_k, vv) in map.iter() {
                if let Some(n) = find_number_in_json(vv, key) {
                    return Some(n);
                }
            }
            None
        ...
        Value::Array(arr) => {
            for item in arr {
                if let Some(n) = find_number_in_json(item, key) {
                    return Some(n);
                }
            }
            None
...
fn find_string_in_json(v: &Value, key: &str) -> Option<String> {
    match v {
        Value::Object(map) => {
            ...
            for (_k, vv) in map.iter() {
                if let Some(s) = find_string_in_json(vv, key) {
                    return Some(s);
                }
            }
            None
        ...
        Value::Array(arr) => {
            for item in arr {
                if let Some(s) = find_string_in_json(item, key) {
                    return Some(s);
                }
            }
            None
...
fn get_estimate(v: &Value, key: &str) -> Option<f64> {
    ...
    if let Some(direct) = find_number_in_json(v, key) {
        return Some(direct);
    }
...
mod tests {
    ...
    fn test_bench_summary_parsing() {
        ...
        fs::write(
            ...
            serde_json::to_string_pretty(&fake_json).unwrap(),
        )
        .unwrap();
